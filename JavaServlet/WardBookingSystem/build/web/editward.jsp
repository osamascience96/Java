...
<html>
    ...
    <%    
        ...
        Ward ward = (Ward) session.getAttribute("ward-edit");
    %>
    <body>
        <div class="header">
        ...
        <% if(ward != null){%>
            <form action="editwardcontroller" method="POST">
                <div class="container rounded bg-white mt-5">
                    <div class="row">
                        <div class="col-md-4 border-right">
                            <div class="d-flex flex-column align-items-center text-center p-3 py-5">
                                <% if(ward.GetImage() != null){
                                    out.println("<img width='180px' height='200px' class='rounded-circle mt-5' src='data:image/png;base64,"+ImageBase64.GetImageData(ward.GetImage())+"'>");
                                }else{
                        ...
                        <div class="col-md-8">
                            <div class="p-3 py-5">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h6 class="text-right">Edit Ward</h6>
                                </div>
                                <div class="row mt-2">
                                    <div class="col">
                                        <input type="text" class="form-control" name="ward-name" placeholder="Ward Name" value="<%=ward.GetName()%>">
                                    </div>
                                </div>
                                <div class="row mt-3">
                                    <div class="col">
                                        ...
                                        <%  
                                            if(ward.GetType().trim().equals("Economy")){
                                                out.println("<option selected value='Economy'>Economy</option>");
                                                out.println("<option value='Economy'>Premium</option>");
                                                out.println("<option value='Emergency'>Emergency</option>");
                                            }else if(ward.GetType().trim().equals("Premium")){
                                                out.println("<option selected value='Premium'>Premium</option>");
                                                out.println("<option value='Economy'>Economy</option>");
                                                out.println("<option value='Emergency'>Emergency</option>");
                                            }else if(ward.GetType().trim().equals("Emergency")){
                                                out.println("<option selected value='Emergency'>Emergency</option>");
                                                out.println("<option value='Economy'>Economy</option>");
                                ...
                                <div class="row mt-3">
                                    <div class="col">
                                        <select name="ward_status" class="custom-select custom-select-lg mb-3">
                                        <%  
                                            if(ward.GetStatus().trim().equals("Booked")){
                                                out.println("<option selected value=1>Booked</option>");
                                                out.println("<option value=0>Not Booked</option>");
                                            }else if(ward.GetStatus().trim().equals("Not Booked")){
                                                out.println("<option selected value=0>Not Booked</option>");
                                                out.println("<option value=1>Booked</option>");
                                            }
                                        %>
                                        </select>
                                    </div>
                                    <div class="col">
                                        <input class="custom-select custom-select-lg mb-3" type="text" name="price_per_day" value="<%=ward.GetPrice()%>" placeholder="Price Per Day">
                                    </div>
                                </div>
                                <h6>Action Timestamp</h6> <p>(Changed Automatically on any update)</p>
                                <div class="row mt-3">
                                    <div class="col-md-6">
                                        <input type="date" class="form-control" value="<%=ward.GetDate()%>">
                                    </div>
                                    <div class="col-md-6">
                                        <input type="time" class="form-control" value="<%=ward.GetTime()%>">
                                    </div>
                                </div>
                                <div class="mt-5 text-right"><input class="btn btn-primary profile-button" type="submit" value="Edit Ward"></div>
                            </div>
                        </div>
                    </div>
                </div>
