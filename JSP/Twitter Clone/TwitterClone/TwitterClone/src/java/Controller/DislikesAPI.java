 ...
 * @author osama
 */
@WebServlet(name = "DislikesAPI", urlPatterns = {"/dislikes_api"})
public class DislikesAPI extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(ratio_type.compareTo("increment") == 0){
            try {
                tweetModel.SetDislikesUpdateIncrement(tweet_id);
            } catch (SQLException ex) {
                Logger.getLogger(LikesAPI.class.getName()).log(Level.SEVERE, null, ex);
            }
        }else {
            try {
                tweetModel.SetDislikesUpdateDecrement(tweet_id);
            } catch (SQLException ex) {
                Logger.getLogger(LikesAPI.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
